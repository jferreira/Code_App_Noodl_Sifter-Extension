...
function contentScriptRequestCallback(request, sender, sendResponse) {
  ...
  if (request.action) {
    ...
    else if (request.action === "yawas_chrome") yawas_chrome(request.color);
...
function setCharactersLeft(txt) {
  ...
  if (highlightswrapper && highlightswrapper.querySelector("#charactersleft"))
    highlightswrapper.querySelector("#charactersleft").textContent =
      txt + "/2048 chars";
}
...
function showNotFound(evt) {
  if (highlightsnotfoundtext.style.display === "none") {
    ...
    highlightsnotfoundtext.innerHTML = `<div style='text-align:left;color:white'>${html.join(
      "<br>"
    )}</div>`;
    highlightsnotfoundtext.style.display = "block";
  } else highlightsnotfoundtext.style.display = "none";
}

function setHighlightsNotFound(array) {
  if (
    highlightswrapper &&
    highlightswrapper.querySelector("#highlightsnotfound")
  ) {
    if (array.length > 0) {
...
function purifyURL(href) {
  if (href && href.indexOf("https://mail.google") === 0) return href;
  var url = href;
  var pos = url.indexOf("#");
  ...
  if (url.indexOf("nytimes.com") != -1) url = url.split("?")[0];
  //console.log('purifyURL=',href,'=',url);
  return url;
}

function yawas_getGoodUrl(doc) {
  //,url)
  var url = null;
  if (
    doc &&
    doc.location &&
    doc.location.hostname.indexOf("readability.com") !== -1
  ) {
    var origin = doc.querySelector(".entry-origin a");
    if (origin) url = origin.href;
  }
...
function yawas_signin() {
  ...
  window.open(
    "https://www.google.com/accounts/ServiceLogin?service=toolbar&nui=1&hl=en&continue=http%3A%2F%2Ftoolbar.google.com%2Fcommand%3Fclose_browser",
    "bkmk_popup",
    "left=" +
      left +
      ",top=" +
...
function yawas_undohighlight() {
  if (lastHighlight !== null) {
    ...
    while (lastHighlight.firstChild) f.appendChild(lastHighlight.firstChild);
...
function addHighlightsWrapper() {
  if (highlightswrapper === null) {
    ...
    highlightswrapper.style.userSelect = "none";
    highlightswrapper.style.fontFamily =
      '"Brandon Text", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"';
    ...
    highlightcaption.addEventListener("mousedown", (evt) =>
      yawas_next_highlight(evt)
    );
...
function yawas_storeHighlight(
  ...
  sendMessage(additionalInfo, function (res) {
    ...
    if ((res && res.undohighlight) || res.error) {
      if (res.signedout)
        alert(
          "Yawas cannot store your highlight because you are signed out.\nPlease signin first and then refresh this page"
        );
      yawas_undohighlight();
    }
  });
...
function recolor(color) {
  if (color === "note") {
    let caption =
      hoverElement.dataset.comment !== null ? hoverElement.dataset.comment : "";
...
function sendMessage(info, cb) {
  try {
    ...
    if (cb) cb({ error: e });
    else console.error("sendMessage error" + e);
  }
}
...
function yawas_chrome(color) {
  ...
  if (elem) {
    ...
    if (color === "note") yawas_tryHighlight(wndWithSelection, true);
    else yawas_tryHighlight(wndWithSelection);
  }
}
        ...
        if (wnd.find(second, true, false)) {
          ...
          if (anchor === s.focusNode) {
            ...
            if (offsetend - offsetstart === len) {
              ...
              highlights[i].selection_unpacted = content.substring(
                offsetstart,
                offsetend
              );
            ...
            if (offsetend - offsetstart === len) {
              var content =
                anchor.textContent.replace(/\s/g, " ") +
                s.focusNode.textContent.replace(/\s/g, " ");
              highlights[i].selection_unpacted = content.substring(
                offsetstart,
                offsetend
              );
...
function highlightDoc(wnd, doc, highlights) {
  ...
  for (var i = 0; i < highlights.length; i++) {
    ...
    if (n == highlights[i].n && wnd.find(selectionString, true, false)) {
      try {
        highlightNowFirefox22(
          wnd.getSelection().getRangeAt(0),
          highlights[i].color,
          forecolor,
          doc,
          highlights[i].selection,
          highlights[i].n,
          highlights[i].comment
        );
        nremapped++;
      } catch (e) {
        console.error("error highlightNowFirefox22", e);
      }
    } else notRemapped.push(highlights[i]);
  }
  wnd.getSelection().removeAllRanges();
  wnd.scrollTo(scrollLeft, scrollTop);
  if (previousRange) wnd.getSelection().addRange(previousRange);
  return nremapped;
}
...
function highlightNowFirefox22(
  selectionrng,
  color,
  textcolor,
  doc,
...
function updateHighlight(elt, color, newcomment) {
  if (elt) {
    sendMessage(
      {
        action: "recolor_highlight",
        url: yawas_getGoodUrl(document),
        title: document.title,
        highlightString: hoverElement.dataset.selection,
        n: hoverElement.dataset.yawasOccurence,
        newcolor: color,
        comment: newcomment,
      },
      function (res) {
        if (res && res.error) {
          //console.error(res);
          yawas_undohighlight();
          if (res.toobig) alert("Too many characters (>2048 even compacted)!");
        } else {
...
function hoverElementOrSelection() {
  if (hoverElement !== null) return hoverElement;
  var wndWithSelection = getWindowWithSelection(window);
  if (!wndWithSelection) return null;
  let rng = wndWithSelection.getSelection().getRangeAt(0);
  let elems = document.querySelectorAll(".yawas-highlight");

  for (let i = 0; i < elems.length; i++) {
    if (rangeIntersectsNode(rng, elems[i].firstChild)) return elems[i];
  }
  return null;
}
...
function computeLength(highlights) {
  var annotation = "";
  for (var i = 0; i < highlights.length; i++) {
    if (highlights[i].comment) annotation += highlights[i].comment;
    annotation += leftMark + highlights[i].selection;
    if (highlights[i].p)
      annotation += "@" + highlights[i].n + "," + highlights[i].p;
    else if (highlights[i].n > 0) annotation += "@" + highlights[i].n;
...
function yawas_delete_highlight() {
  ...
  if (elem) {
    sendMessage(
      {
        ...
        url: yawas_getGoodUrl(document),
        title: document.title,
        highlightString: elem.dataset.selection,
        n: elem.dataset.yawasOccurence,
      },
      function (res) {
        if (res && res.highlights)
          setCharactersLeft(computeLength(res.highlights));
    ...
    var f = document.createDocumentFragment();
    while (elem.firstChild) f.appendChild(elem.firstChild);
...
function yawas_next_highlight(evt) {
  ...
  if (!signedin) return yawas_signin();
...
if (
  document.location.hostname === "toolbar.google.com" &&
  document.location.pathname === "/command" &&
  document.location.search &&
  document.location.search.indexOf("close_browser") !== -1
) {
  window.close();
  sendMessage({ fn: "yawas_toolbar_signed_in" });
} else if (
  document.location.href.indexOf("accounts.google.com/ServiceLogin") != -1
) {
  ...
  if (window.top !== window) {
    ...
    addStyle(
      window.document,
      ".yawas-highlight:hover{opacity:0.6;}.yawas-highlight[data-comment]{border-bottom:1px dashed black}"
    );
    askForAnnotations(2000);
  }
